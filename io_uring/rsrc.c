...
int __io_account_mem(struct user_struct *user, struct pins_cgroup *pins_cg,
		unsigned long nr_pages)
{
	if (!nr_pages)
		return 0;

	if (account_locked_user_vm(user, pins_cg, nr_pages))
		return -ENOMEM;
...
{
	if (ctx->user)
		unaccount_locked_user_vm(ctx->user, ctx->pins_cg, nr_pages);
